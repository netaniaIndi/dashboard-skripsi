...
if menu == "🏠 Home":
    ...
    with col1:
        st.subheader("🏥 Ulasan per Rumah Sakit")
...
# Menu Clustering
    ...
    with col1:
        st.metric("Total Review", len(filtered_df_cluster))
    
    with col2:
        st.metric("Total Cluster", total_clusters)
    ...
    # Cluster dengan rating terbaik dan terburuk
    overall_cluster_rating = filtered_df_cluster.groupby('cluster')['rating'].mean().round(2)
    best_cluster = overall_cluster_rating.idxmax()
    worst_cluster = overall_cluster_rating.idxmin()
    
    # Hospital dengan clustering terbaik
    hospital_cluster_diversity = filtered_df_cluster.groupby('location')['cluster'].nunique()
    most_diverse_hospital = hospital_cluster_diversity.idxmax()
    least_diverse_hospital = hospital_cluster_diversity.idxmin()
